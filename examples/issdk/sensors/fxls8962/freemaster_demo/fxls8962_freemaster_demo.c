 ...
 *  @return          void.
 */
void fxls896x_isr_callback(ARM_GPIO_Pin_t pin, uint32_t event);
...
int app_main(void)
{
    ...
    /*! Setup FXLS8962_INT1 pin used by board */
    pGpioDriver->Setup(FXLS8962_INT1, &fxls896x_isr_callback);
...
int32_t perform_selftest(fxls8962_i2c_sensorhandle_t fxls8962Driver, fxls896x_selftest_t *selftest)
{
	...
	while (axis<6)
	{
		...
		for(counter=0;counter<1;counter++)
		{
			...
			Register_I2C_Write(&FXLS8962_I2C_DRIVER,&(fxls8962Driver.deviceInfo),FXLS8962_I2C_ADDR, FXLS8962_SENS_CONFIG1,0x00,0x00,0x00);
			if (ARM_DRIVER_OK != status)
			{
				return SENSOR_ERROR_WRITE;
			}
