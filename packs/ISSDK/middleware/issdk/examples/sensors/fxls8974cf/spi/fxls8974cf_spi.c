...
int app_main(void)
{
    ...
    status = pSPIdriver->Control(ARM_SPI_MODE_MASTER | ARM_SPI_CPOL0_CPHA0 | ARM_SPI_DATA_BITS(8), FXLS8974_SPI_BAUDRATE);
    if (ARM_DRIVER_OK != status)
    {
    ...
    FXLS8974_SPI_SetIdleTask(&fxls8974Driver, (registeridlefunction_t)COMM_IDLE_FUNC, COMM_IDLE_ARG);
