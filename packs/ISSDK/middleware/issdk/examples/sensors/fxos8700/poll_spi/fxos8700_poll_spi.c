...
int app_main(void)
{
    ...
    status = pSPIdriver->Control(ARM_SPI_MODE_MASTER | ARM_SPI_CPOL0_CPHA0 | ARM_SPI_DATA_BITS(8), FXOS8700_SPI_BAUDRATE);
    if (ARM_DRIVER_OK != status)
    {
    ...
    status = FXOS8700_SPI_Initialize(&fxos8700Driver, &FXOS8700_SPI_DRIVER, FXOS8700_SPI_INDEX, FXOS8700_SPI_CS,
    ...
    FXOS8700_SPI_SetIdleTask(&fxos8700Driver, (registeridlefunction_t)COMM_IDLE_FUNC, COMM_IDLE_ARG);
