...
/**
 * @file fxls8961_drv.c
 * @brief The fxls8961_drv.c file implements the FXLS8961AF (Chiron S&C) sensor driver interfaces.
 */
...
// Global Variables
//-----------------------------------------------------------------------
uint8_t fxls8961_spiRead_CmdBuffer[FXLS8961_SPI_MAX_MSG_SIZE] = {0};
uint8_t fxls8961_spiRead_DataBuffer[FXLS8961_SPI_MAX_MSG_SIZE] = {0};
uint8_t fxls8961_spiWrite_CmdDataBuffer[FXLS8961_SPI_MAX_MSG_SIZE] = {0};
...
void FXLS8961_SPI_ReadPreprocess(void *pCmdOut, uint32_t offset, uint32_t size)
{
    ...
    uint8_t *pWBuff = fxls8961_spiRead_CmdBuffer;
    uint8_t *pRBuff = fxls8961_spiRead_DataBuffer;

    /* Formatting for Read command of FXLS8961 SENSOR. */
    ...
    *(pWBuff + 1) = 0x00;
    ...
    pSlaveCmd->size = size + FXLS8961_SPI_CMD_LEN;
...
void FXLS8961_SPI_WritePreprocess(void *pCmdOut, uint32_t offset, uint32_t size, void *pWritebuffer)
{
    ...
    uint8_t *pWBuff = fxls8961_spiWrite_CmdDataBuffer;
    uint8_t *pRBuff = fxls8961_spiWrite_CmdDataBuffer + size + FXLS8961_SPI_CMD_LEN;

    /* Formatting for Write command of FXLS8961 SENSOR. */
    ...
    *(pWBuff + 1) = 0x00;
    ...
    memcpy(pWBuff + FXLS8961_SPI_CMD_LEN, pWritebuffer, size);
...
int32_t FXLS8961_SPI_Initialize(
    fxls8961_spi_sensorhandle_t *pSensorHandle, ARM_DRIVER_SPI *pBus, uint8_t index, uint32_t SlaveSelect, uint8_t *whoami)
{
    int32_t status;
    uint8_t reg;
    ...
    pSensorHandle->slaveParams.pReadPreprocessFN = FXLS8961_SPI_ReadPreprocess;
    pSensorHandle->slaveParams.pWritePreprocessFN = FXLS8961_SPI_WritePreprocess;
    pSensorHandle->slaveParams.TargetSlavePinID = SlaveSelect;
    pSensorHandle->slaveParams.spiCmdLen = FXLS8961_SPI_CMD_LEN;
    pSensorHandle->slaveParams.ssActiveValue = FXLS8961_SS_ACTIVE_VALUE;
...
int32_t FXLS8961_SPI_Configure(fxls8961_spi_sensorhandle_t *pSensorHandle, const registerwritelist_t *pRegWriteList)
{
    ...
    status = Register_SPI_Write(pSensorHandle->pCommDrv, &pSensorHandle->deviceInfo, &pSensorHandle->slaveParams,
                                FXLS8961_SENS_CONFIG1, FXLS8961_SENS_CONFIG1_ACTIVE_STANDBY,
                                FXLS8961_SENS_CONFIG1_ACTIVE_MASK);
    if (ARM_DRIVER_OK != status)
    {
    ...
    status = Register_SPI_Write(pSensorHandle->pCommDrv, &pSensorHandle->deviceInfo, &pSensorHandle->slaveParams,
                                FXLS8961_SENS_CONFIG1, FXLS8961_SENS_CONFIG1_ACTIVE_ACTIVE,
                                FXLS8961_SENS_CONFIG1_ACTIVE_MASK);
    if (ARM_DRIVER_OK != status)
    {
        return SENSOR_ERROR_WRITE;
...
int32_t FXLS8961_SPI_Deinit(fxls8961_spi_sensorhandle_t *pSensorHandle)
{
    ...
    status = Register_SPI_Write(pSensorHandle->pCommDrv, &pSensorHandle->deviceInfo, &pSensorHandle->slaveParams,
                                FXLS8961_SENS_CONFIG1, FXLS8961_SENS_CONFIG1_RST_RST, FXLS8961_SENS_CONFIG1_RST_MASK);
    if (ARM_DRIVER_OK != status)
    {
        return SENSOR_ERROR_WRITE;
    }
     ...
     * to allow enough time for FXLS8961AF to complete its internal boot sequence and be ready for communication. */
    BOARD_DELAY_ms(FXLS8961_TBOOT_MAX);
    ...
    fxls8961_i2c_sensorhandle_t *pSensorHandle, ARM_DRIVER_I2C *pBus, uint8_t index, uint16_t sAddress, uint8_t *whoami)
{
    ...
    /*!  Read and store the device's WHO_AM_I.*/
    status = Register_I2C_Read(pBus, &pSensorHandle->deviceInfo, sAddress, FXLS8961_WHO_AM_I, 1, &reg);
    //if ((ARM_DRIVER_OK != status) || (whoAmi != reg))
	(*whoami) = reg;
    if (ARM_DRIVER_OK != status)
    {
...
int32_t FXLS8961_I2C_Configure(fxls8961_i2c_sensorhandle_t *pSensorHandle, const registerwritelist_t *pRegWriteList)
{
    ...
    status = Register_I2C_Write(pSensorHandle->pCommDrv, &pSensorHandle->deviceInfo, pSensorHandle->slaveAddress,
                                FXLS8961_SENS_CONFIG1, FXLS8961_SENS_CONFIG1_ACTIVE_STANDBY,
                                FXLS8961_SENS_CONFIG1_ACTIVE_MASK, false);
    if (ARM_DRIVER_OK != status)
    {
        return SENSOR_ERROR_WRITE;
    }
...
int32_t FXLS8961_I2C_DeInit(fxls8961_i2c_sensorhandle_t *pSensorHandle)
{
    ...
    status =
        Register_I2C_Write(pSensorHandle->pCommDrv, &pSensorHandle->deviceInfo, pSensorHandle->slaveAddress,
                           FXLS8961_SENS_CONFIG1, FXLS8961_SENS_CONFIG1_RST_RST, FXLS8961_SENS_CONFIG1_RST_MASK, false);
    if (ARM_DRIVER_OK != status)
    {
        return SENSOR_ERROR_WRITE;
    }
