...
/**
 * @file fxls896x_drv.h
 * @brief The fxls896x_drv.h file describes the FXLS8964/67AF driver interface and structures.
 */

#ifndef FXLS896X_AF_H_
#define FXLS896X_AF_H_
...
} fxls896x_i2c_sensorhandle_t;

/*! @brief This structure defines the fxls896x raw data buffer.*/
typedef struct
{
    uint32_t timestamp; /*! The time, this sample was recorded.  */
    int16_t accel[3];   /*!< The accel data */
} fxls896x_acceldata_t;

/*! @def    fxls896x_SPI_MAX_MSG_SIZE
 *  @brief  The MAX size of SPI message. */
#define FXLS896x_SPI_MAX_MSG_SIZE (64)

/*! @def    FXLS896x_SPI_CMD_LEN
 *  @brief  The size of the Sensor specific SPI Header. */
#define FXLS896x_SPI_CMD_LEN (2)

/*! @def    FXLS896x_SS_ACTIVE_VALUE
 *  @brief  Is the Slave Select Pin Active Low or High. */
#define FXLS896x_SS_ACTIVE_VALUE SPI_SS_ACTIVE_LOW
 ...
 *  @reeentrant  No
 *  @return      ::FXLS896x_I2C_Initialize() returns the status .
 */
 ...
 *  @param[in]  :  fxls896x_i2c_sensorhandle_t *pSensorHandle, handle to the sensor handle.
 ...
 *  @return        void.
 *  @constraints   This can be called any number of times only after FXLS896x_I2C_Initialize().
 *                 Application has to ensure that previous instances of these APIs have exited before invocation.
 *  @reeentrant    No
 */
 ...
 *               Application has to ensure that previous instances of these APIs have exited before invocation.
 *  @reeentrant  No
 *  @return      ::FXLS896x_I2C_Configure() returns the status .
 */
 ...
 *               Application has to ensure that previous instances of these APIs have exited before invocation.
 *  @reeentrant  No
 *  @return      ::FXLS896x_I2C_ReadData() returns the status .
 */
 ...
 *  @param[in]   pSensorHandle handle to the sensor.
 *  @constraints This can be called only after FXLS896x_I2C_Initialize().
 *               Application has to ensure that previous instances of these APIs have exited before invocation.
 *  @reeentrant  No
 *  @return      ::FXLS896x_I2C_DeInit() returns the status .
 */
 ...
 *               Application has to ensure that previous instances of these APIs have exited before invocation.
 *  @reeentrant  No
 *  @return      ::FXLS896x_SPI_Initialize() returns the status .
 */
int32_t FXLS896x_SPI_Initialize(fxls896x_spi_sensorhandle_t *pSensorHandle,
                                ARM_DRIVER_SPI *pBus,
                                uint8_t index,
 ...
 *  @param[in]  :  fxls896x_spi_sensorhandle_t *pSensorHandle, handle to the sensor handle.
 *  @param[in]  :  registeridlefunction_t idleTask, function pointer to the function to execute on SPI Idle Time.
 *  @param[in]  :  void *userParam, the pointer to the user idle ftask parameters.
 *  @return        void.
 *  @constraints   This can be called any number of times only after FXLS896x_SPI_Initialize().
 *                 Application has to ensure that previous instances of these APIs have exited before invocation.
 *  @reeentrant    No
 */
 ...
 *               Application has to ensure that previous instances of these APIs have exited before invocation.
 *  @reeentrant  No
 *  @return      ::FXLS896x_SPI_Configure() returns the status .
 */
 ...
 *               Application has to ensure that previous instances of these APIs have exited before invocation.
 *  @reeentrant  No
 *  @return      ::FXLS896x_SPI_ReadData() returns the status .
 */
 ...
 *  @details     This function made sensor in a power safe state and de initialize its handle.
 *  @param[in]   pSensorHandle      handle to the sensor.
 *  @constraints This can be called only after after FXLS896x_SPI_Initialize().
 *               Application has to ensure that previous instances of these APIs have exited before invocation.
 *  @reeentrant  No
 *  @return      ::FXLS896x_SPI_Deinit() returns the status .
 */
